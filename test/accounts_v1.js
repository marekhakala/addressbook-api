...
describe("Accounts", () => {
  ...
  let accountParams = { account: { fullname: "Demo Demo", email: "demo@demo.com", password: "demodemo" } };
  let accountEditParams = { account: { fullname: "Edit Edit", email: "edit@edit.com", password: "editedit" } };
  let accountSecondParams = { account: { fullname: "Demo Demo", email: "seconddemo@demo.com", password: "abc123abc" } };
  ...
  describe("PUT /api/v1/account", () => {
    it("it should receive a token error", done => {
      chai.request(server)
        ...
        .end((err, res) => {
          res.should.have.status(401);
          res.body.should.have.property("status");
          res.body.status.should.not.be.null;
			  	res.body.should.have.property("errors");
          done();
        });
    });
    ...
    it("it should receive an updated fullname", done => {
			chai.request(server)
      .put("/api/v1/account")
      .set("authorization", "Token " + apiToken)
      .send({ account: { fullname: accountEditParams["account"]["fullname"],
  ...
  describe("POST /api/v1/accounts", () => {
    ...
    it("it should receive an error - duplicate email address", done => {
      chai.request(server)
      .post("/api/v1/accounts")
      .send({ account: { fullname: accountSecondParams["account"]["fullname"],
       email: accountSecondParams["account"]["email"],
